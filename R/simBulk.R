...
#'
#' Generate training and test cell composition matrices for the simulation of bulk
#' samples with known cell composition using single-cell expression profiles.
#' The resulting matrix will determine the proportion of the different cell types
#' that will form the simulated bulk samples.
#'
...
#' due to the composition of bulk samples, proportions for the mixtures
#' (bulk samples) of cell types (\eqn{w_1,...,w_k}, where \eqn{k} is the number
#' of cell types available in single-cell profiles), are randomly generated using
#' five different approaches:
#'
#' \enumerate{
#'   \item Cell proportions are randomly sampled from a truncated uniform
#'   distribution with predefined limits according to a priori knowledge of the
#'   abundance of each cell type (see \code{prob.design} argument). This
#'   information ban be inferred from the single cell analysis itself or from
#'   the literature.
#'   \item A second set is generated by randomly permuting cell type labels from
#'   a distribution generated by the previous method.
#'   \item Cell proportions are randomly sampled as by method 1 without
#'   replacement.
#'   \item Using the last method for generating proportions, cell types labels
#'   are randomly sampled.
#'   \item Cell proportions are randomly sampled from a Dirichlet distribution.
#' }
#'
#' If you want to see the distribution of cell type proportions generated by each
#' method during the process, you can access them with \code{\link{showProbPlot}}
#' function (see examples).
#'
#' It is important to note that the number of bulk-samples simulated are determined
#' in this step. You can predefine the number of bulk profiles generated using
#' \code{num.bulk.samples} argument. By default, the number of bulk samples
#' generated depends on the number of single-cell profiles available: approximately
#' 18 more samples will be formed than there are cells in \code{single.cell.final}.
#' We recommend set a number of 30000 samples. This number will be split in training
#' and test data: 60% for training and 40% for evaluating the model.
#
...
#' @param prob.design \code{data.frame} with the frequency ranges expected
#' for each cell type present in the experiment. This information can be estimated from
...
#' type.
#' \item A third column named 'to' with the final frequency for each cell type.}
#' @param proportions.train Vector of five integer numbers that determine the proportions
#' of bulk samples that will be generated by the methods explained in details in
#' train samples. This vector represents proportions, so they must add 100 and
#' none can be less than 1. By default, a majority of random samples without
...
#'
#' @return A \code{\link{DigitalDLSorter}} object with \code{prob.cell.types} slot containing
...
#' important element is the cell composition matrix, which is formed by \eqn{n} rows
...
#' built by \code{\link{generateTrainAndTestBulkProbMatrix}} function.
...
#' the probability matrix. In practice, as described in Torroja et al., 2019,
#' these profiles are generated by the summation
#' of 100 cells from different cell types determined by cell composition matrix.
#' The number of bulk samples is determined by dimensions of cell composition
#' matrix. See \code{\link{generateTrainAndTestBulkProbMatrix}} for details.
#'
#' \code{digitalDLSorteR} allows the use of HDF5 files as back-end for the resulting data
#' using \code{DelayedArray} and \code{HDF5Array} packages in cases of generating
#' too large bulk expression matrix. This functionality allows you to work without
#' keeping the data loaded in memory, which will be of vital importance during
#' some computationally heavy steps such as neural network training. You must provide
#' a valid file path in \code{file.backend} argument to store the resulting
#' file with '.h5' extension. The data
...
#' option due to the large size of the simulated matrices.
#'
...
#' (2 by default).
#' @param compression.level The compression level used if file.backend provided (6 by default).
#' It is an integer value between 0 (no compression) and 9 (highest and
#' slowest compression).
#' @param verbose Show informative messages during the execution.
#'
#' @return A \code{\link{DigitalDLSorter}} object with \code{bulk.sim} slot containing
...
#' Prepare training and test final data for training and evaluation
...
#' in log2-space and normalized. Samples are shuffled in order to
#' avoid biases during training. Note that expression matrix is transposed
#' in order to prepare data for training.
#'
#' This function allows you to select which kind of data you want to
#' use for training: single-cell profiles, bulk profiles or a combination of both.
#' See \code{combine} argument for details. We recommend the use of the combination
...
#' you to work without keeping the data loaded in memory, which will be of
#' vital importance during some computationally heavy steps such as neural network
#' training. You must provide a valid file path in \code{file.backend} argument
#' to store the resulting file with
#' '.h5' extension. The data will be accessible from R without being loaded into
#' memory. This option slightly slows down execution times, since subsequent
#' transformations of data will be carried out by chunks instead of using all
#' data. We recommend this option due to the large size of the simulated matrices.
#'
#' @param object \code{\link{DigitalDLSorter}} object with \code{single.cell.final} and
#' \code{prob.cell.types} slots.
#' @param type.data Type of data to generate among 'train', 'test' or 'both'
#' (the last by default).
#' @param combine Character determining if combine training data. Can be 'both',
#' 'bulk' or 'single-cell' ('both' by default). Note that test data is always combined.
#' @param file.backend A valid file path where to save the HDF5 file used as
#' back-end. If it is equal to \code{NULL} (by default), the data are loaded in memory.
...
#' \code{\link{trainDigitalDLSorterModel}} only access to data by columns. You can
#' provided the number of columns that are stored together in each chunk. Note
#' that the more columns the more RAM is used, although time times are improved.
#' @param verbose Show informative messages during the execution.
#'
#' @return A \code{\link{DigitalDLSorter}} object with \code{final.data} slot containing
#' a list with one or two entries (depending on selected \code{type.data} argument):
#' 'train' and 'test'. Each entry contains a \code{SummarizedExperiment} object
