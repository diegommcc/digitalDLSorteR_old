...
#'
#' The ProbMatrixCellTypes class is a data storage class that stores
#' information concerning to cell composition matrix used for the simulation
#' of bulk samples. This matrix corresponds
#' with \code{prob.matrix} slot. The rest of slots are additional
#' information generated during the process and required for subsequent steps.
#'
#' As described in Torroja and Sanchez-Cabo, 2019,
#' the proportions are built by five different methods in order to avoid biases
#' due to the composition of the bulk samples. In \code{plots} slot,
#' different representations of these probabilities are stored
#' with the aim of offering a method to monitor
#' the different sets of samples generated during the process. These plots can
#' be displayed with \code{\link{showProbPlot}} function. See documentation for
#' details.
#'
...
#' columns are the cell types present in single-cell data provided (\eqn{j}) and
#' each entry is the proportion of \eqn{j} cell type in \eqn{i} sample.
#' @slot cell.names Matrix in which names of cells that will compound each
#' bulk samples are stored.
...
#' If you want to save the object in an rds file, \code{digitalDLSorteR} provides
#' an \code{saveRDS} generic that transforms the keras model into a native valid
#' R object. Specifically, the model is converted into a list with the architecture
#' of the network and the weights learned during the training. The is the
#' minimum information to use the model as predictor. If you want to maintain
#' the optimizer state, see \code{\link{saveTrainedModelAsH5}} function. If you
#' want to store an object as rda file, see \code{\link{preparingToSave}} function.
#'
#' @slot model Trained Deep Neural Network model. This slot can contain a
#' R6 \code{keras.engine.sequential.Sequential} object or a list with two
...
#' @slot predict.results Deconvolution results matrix of test data.
...
#' @slot eval.stats.samples Performance of the model on each sample of
#' test data in comparison with the known cell proportions.
#'
...
#' on which to carry out the process of devonvolution. Only it is used in the
#' case of building new deconvolution models. For deconvoluting bulk samples
...
#' during the process:
#' \itemize{
#' \item \code{SingleCellExperiment} class for single-cell RNA-seq data, using
#'  sparse matrix from the \code{Matrix} package (\code{dgCMatrix} class) to store
#'  the matrix of counts.
#' \item \code{ZinbParams} class with the estimated parameters for the simulation of
#' new single-cell profiles.
#' \item \code{SummarizedExperiment} class for storing bulk RNA-seq data. In this
#'  case, it is possible to load all data in memory or the use of HDF5 files as
#'  back-end by \code{DelayedArray} and \code{HDF5Array} packages.
#'  See \code{\link{generateBulkSamples}} for details.
#' \item \code{\link{ProbMatrixCellTypes}} class for the composition cell matrices
#' built during the process. See \code{?ProbMatrixCellTypes} for details.
#' \item \code{\link{DigitalDLSorterDNN}} class for storing the trained Deep
#' Neural Network. This step is performed by \code{keras}.
#' See \code{\link{DigitalDLSorterDNN}} for details.
#' }
#'
#' @slot single.cell.real Real single-cell data stored in a \code{SingleCellExperiment}
#' object. The counts matrix is stored as a \code{dgCMatrix} object to optimize
...
#' @slot single.cell.final Final single-cell expression profiles used for
...
#' composition  matrix built for the simulation of bulk RNA-seq profiles.
#' The entries determine the proportion of single-cell types that will
#' constitute the simulated bulk samples.
#' @slot bulk.sim A list with two elements: train and test simulated bulk RNA-seq.
#' This data are stored as a \code{SummarizedExperiment} object. We recommend the
#' use of HDF5 file as a back-end due to the large amount of memory that they occupy.
#' @slot final.data The final data that will be used for training and testing the
#' Deep Neural Network. As in the previous slot, it is a list with two items, train
#' and test. With respect to train counts matrix, it can be the train bulk RNA-seq
#' samples, the train scRNA-seq samples or a combination of both. In the case of
#' test counts matrix, RNA-seq data from bulk and single-cell will be combined.
#' Moreover, data is scaled and shuffled for training.
#' @slot trained.model \code{\link{DigitalDLSorterDNN}} object with the trained model,
#' different metrics obtained during the training and evaluation metrics from
#' the application of the model on test data. After executing
#' \code{\link{calculateEvalMetrics}}, it is alto possible to find the results
#' of the model evaluation.
#' @slot deconv.data Optional slot where is possible to store new bulk samples for
#' its deconvolution. It is a list whose name is the name of the data provided.
#' It is possible to store more than one dataset to make predictions.
#' See \code{\link{deconvDigitalDLSorterModelObj}} for details.
...
#'
#' The package can be used in two ways: to build new models of deconvolution from
#' scRNA-seq data or to deconvolute bulk RNA-seq samples using pre-trtained
#' models integrated into the package. If you want to build new models,
#' see \code{\link{loadRealSCProfiles}} or \code{\link{loadFinalSCProfiles}}
#' functions. If yoy want to use pre-trained models, see
...
DigitalDLSorter <- setClass(
  ...
  slots = c(
    ...
    single.cell.final = "SingleCellExperimentOrNULL",
...
setMethod(
  ...
  definition = function(
    ...
    .Object@single.cell.final <- single.cell.final
...
setMethod(f = "show",
          ...
          definition = function(object) {
            ...
            if (!is.null(object@single.cell.final)) {
              cat("Final single-cell profiles:\n")
              .sceShow(object@single.cell.final)
            }
